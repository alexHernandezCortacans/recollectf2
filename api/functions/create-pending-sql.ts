// api/functions/create-pending-sql.ts
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === "GET") {
    return res.status(200).json({ whoami: "CREATE-PENDING-SQL" });
  }
  ...
  const { inputs, sqlPath } = req.body || {};

  if (!inputs?.queries) return res.status(400).json({ error: "Missing inputs.queries" });
  if (!sqlPath) return res.status(400).json({ error: "Missing sqlPath" });
